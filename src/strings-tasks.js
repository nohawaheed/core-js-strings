...
function orderAlphabetically(str) {
  const orderedArray = str.split('').sort((a, b) => (a > b ? 1 : -1));
...
function countVowels(str) {
  ...
  const matchedCount = str.match(vowelsRegex);
  return matchedCount?.length ?? 0;
}
...
function isPalindrome(str) {
  ...
  reversedString = reversedString.toLowerCase().replace(regex, '');
  return reversedString === str.toLowerCase().replace(regex, '');
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    if (words[i].length > longestWrord.length) {
      longestWrord = words[i];
    }
  }
  return longestWrord;
}
...
function reverseWords(str) {
  ...
  return words.map((value) => value.split('').reverse().join('')).join(' ');
}
...
function invertCase(str) {
  ...
  const invertedString = str.split('').map((value) => {
...
function encodeToRot13(str) {
  const encodedString = str.split('').map((value) => {
    if (!value.match(/[A-Za-z]/)) return value;
    if (value.match(/[A-Z]/g) && value.charCodeAt(0) + 13 > 90) {
      return String.fromCharCode(value.charCodeAt(0) + 13 - 26);
    }
    if (value.match(/[a-z]/g) && value.charCodeAt(0) + 13 > 122) {
      return String.fromCharCode(value.charCodeAt(0) + 13 - 26);
    }
...
function getCardId(value) {
  ...
  const suits = ['♣', '♦', '♥', '♠'];
  const cardValue = values.indexOf(value.slice(0, -1));
  const cardSuit = suits.indexOf(value.slice(-1)) * 13;
